...
final class Line {
    ...
    init(sceneView: ARSCNView, startVector: SCNVector3, unit: DistanceUnit) {
        ...
        text = SCNText(string: "", extrusionDepth: 0)
...
//        return node
        ...
        let node = SCNNode().buildLineInTwoPointsWithRotation(from: self, to: vector, radius: 0.002, color: .white)
        return node
    }
}
